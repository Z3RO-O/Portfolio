    ...
    <div className='w-full py-4'>
      {skills.map((category, catIndex) => (
        <div key={catIndex} className='mb-8'>
          <h4 className='text-2xl font-semibold text-(--secondary) mb-4 pb-2'>
            {category.category}
          </h4>
          <div className='grid grid-cols-[repeat(auto-fit,minmax(120px,1fr))] gap-6 py-4 md:grid-cols-[repeat(auto-fit,minmax(100px,1fr))] md:gap-4 sm:grid-cols-[repeat(auto-fit,minmax(80px,1fr))] sm:gap-3'>
            {category.items.map((skill, index) => (
              ...
              >
                ...
                <span className='text-sm md:text-[0.8rem] sm:text-xs font-semibold text-(--secondary) text-center leading-[1.2] relative z-1'>
                  {skill.name}
                </span>
              </div>
            ))}
